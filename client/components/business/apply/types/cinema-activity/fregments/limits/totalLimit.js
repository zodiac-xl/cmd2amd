...
export default class TotalLimit extends SuperChild {
    ...
    renderMain() {
        ...
        let rightDom;
        if (readOnly) {
            rightDom = <Right><span>{totalLimit > 0 ? `${totalLimit}张` : '不限'}</span></Right>;
        } else {
            rightDom = <Right>
                <RadioGroup name={'totalLimit'} value={radioValue}
