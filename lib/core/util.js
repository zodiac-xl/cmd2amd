...
function getModulePath(source, relatePath, rootPath) {
    ...
    if (/([.]\/)/.test(source)) {
        ...
        if (!_pathExists2.default.sync(realPath)) {} else {
            ...
            if (_pathExists2.default.sync(pk)) {
                ...
                if (main) {
                    if (/\.js$/.test(main)) {
                        if (_pathExists2.default.sync(_path2.default.join(realPath, main))) {
                            realPath = _path2.default.join(realPath, main);
                        }
                    } else {
                        if (_pathExists2.default.sync(_path2.default.join(realPath, main + '.js'))) {
                            realPath = _path2.default.join(realPath, main + '.js');
                        }
                    }
                }
            } else if (_pathExists2.default.sync(_path2.default.join(realPath, 'index.js'))) {
    ...
    if (!_path2.default.parse(realPath).ext) {
        ...
        var realIndexJsx = realPath + '/index.jsx';
        if (_pathExists2.default.sync(realJS)) {
            ...
            rs.push(realIndex);
        } else if (_pathExists2.default.sync(realIndexJsx)) {
